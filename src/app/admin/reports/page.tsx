  ...
  useEffect(() => {
    const load = async () => {
      ...
      try {
        ...
        setRevenueReport(res.data.data.revenue);
        setAuctionReport(res.data.data.auctions);
        setUserReport(res.data.data.users);
    ...
    <div className="space-y-6">
      ...
      <div className="card p-4">
        <div className="flex flex-col sm:flex-row items-end gap-3">
          ...
          <button onClick={fetchWithDates} className="btn-primary !py-2.5">
            Apply Filter
          </button>
        </div>
      </div>
              ...
              <tbody className="divide-y">
                {revenueReport.byMonth.map((m: any, i: number) => (
                  <tr key={i} className="hover:bg-gray-50">
                    ...
                    <td className="px-4 py-2">
                      {formatCurrency(m.revenue / (m.count || 1))}
                    </td>
                  </tr>
                ))}
              </tbody>
