...
export default function AdminDashboard() {
  ...
  const cards = [
    ...
    { label: 'Total Revenue', value: formatCurrency(stats?.totalRevenue || 0), icon: DollarSign, color: 'bg-burgundy' },
          ...
          <div className="space-y-3">
            {(stats?.recentUsers || []).map((u: any) => (
              <div key={u._id} className="flex items-center justify-between py-2 border-b last:border-0">
                <div>
                  <p className="font-medium text-base">{u.firstName} {u.lastName}</p>
                  <p className="text-sm text-gray-500">{u.email}</p>
                </div>
                <span className={`text-sm px-2 py-1 rounded-full ${u.role === 'client' ? 'bg-purple-100 text-purple-700' : 'bg-blue-100 text-blue-700'}`}>{u.role}</span>
              </div>
            ))}
            {(!stats?.recentUsers || stats.recentUsers.length === 0) && <p className="text-base text-gray-500">No recent users</p>}
          </div>
        </div>
          ...
          <div className="space-y-3">
            {(stats?.recentOrders || []).map((o: any) => (
              <div key={o._id} className="flex items-center justify-between py-2 border-b last:border-0">
                <div>
                  <p className="font-medium text-base">{o.orderNumber}</p>
                  <p className="text-sm text-gray-500">{o.lot?.title || 'N/A'}</p>
                </div>
                <div className="text-right">
                  <p className="font-semibold text-base">{formatCurrency(o.totalAmount)}</p>
                  <span className={`text-sm px-2 py-1 rounded-full ${o.paymentStatus === 'paid' ? 'bg-green-100 text-green-700' : 'bg-yellow-100 text-yellow-700'}`}>{o.paymentStatus}</span>
                </div>
              </div>
            ))}
            {(!stats?.recentOrders || stats.recentOrders.length === 0) && <p className="text-base text-gray-500">No recent orders</p>}
          </div>
        </div>
      </div>
    </div>
