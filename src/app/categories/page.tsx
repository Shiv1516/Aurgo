        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {categories.map((cat) => (
            <Link key={cat._id} href={`/categories/${cat.slug}`}>
              <div className="relative group rounded-xl overflow-hidden bg-navy h-48 flex items-center justify-center hover:shadow-xl transition-all">
                {cat.image ? (
                  <div
                ...
                <div className="relative text-center z-10 p-4">
                  <h3 className="text-white font-heading font-black text-2xl mb-1 uppercase tracking-wider">
                    {cat.name}
                  </h3>
                  {cat.description && (
                    <p className="text-gray-300 text-sm font-medium line-clamp-2">{cat.description}</p>
                  )}
                </div>
              </div>
            </Link>
