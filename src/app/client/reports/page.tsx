...
export default function ClientReportsPage() {
  ...
  return (
    <div>
      <h1 className="text-3xl font-heading font-bold text-dark mb-6">Reports & Analytics</h1>
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-8">
        <div className="card p-5"><div className="flex items-center justify-between"><div><p className="text-base text-gray-500">Total Revenue</p><p className="text-3xl font-bold text-dark mt-1">{formatCurrency(stats.totalRevenue || 0)}</p></div><DollarSign className="h-8 w-8 text-burgundy" /></div></div>
        <div className="card p-5"><div className="flex items-center justify-between"><div><p className="text-base text-gray-500">Total Orders</p><p className="text-3xl font-bold text-dark mt-1">{stats.totalOrders || 0}</p></div><ShoppingBag className="h-8 w-8 text-blue-500" /></div></div>
        <div className="card p-5"><div className="flex items-center justify-between"><div><p className="text-base text-gray-500">Avg Order Value</p><p className="text-3xl font-bold text-dark mt-1">{formatCurrency(stats.avgOrderValue || 0)}</p></div><TrendingUp className="h-8 w-8 text-green-500" /></div></div>
      </div>
