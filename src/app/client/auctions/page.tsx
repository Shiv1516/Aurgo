  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-3xl font-heading font-bold text-dark">My Auctions</h1>
        <Link href="/client/auctions/new" className="btn-primary text-base !py-2.5 gap-2"><Plus className="h-4 w-4" /> Create Auction</Link>
      </div>
        ...
        <div className="card overflow-hidden"><div className="overflow-x-auto"><table className="w-full"><thead><tr className="table-header"><th className="px-4 py-3">Auction</th><th className="px-4 py-3">Status</th><th className="px-4 py-3">Lots</th><th className="px-4 py-3">Bids</th><th className="px-4 py-3">Dates</th><th className="px-4 py-3">Actions</th></tr></thead><tbody>
          {auctions.map((a: any) => (
            <tr key={a._id} className="border-t border-gray-50 hover:bg-gray-50">
              ...
              <td className="px-4 py-3 text-base">{a.totalBids}</td>
              <td className="px-4 py-3 text-sm text-gray-500">{formatDate(a.startTime)} - {formatDate(a.endTime)}</td>
              <td className="px-4 py-3">
                <div className="flex items-center gap-2">
                  <Link href={`/client/auctions/${a._id}/lots`} className="text-burgundy hover:text-burgundy-dark text-base">Lots</Link>
                  <Link href={`/client/auctions/${a._id}/edit`} className="text-gray-500 hover:text-gray-700"><Edit className="h-4 w-4" /></Link>
                  <button onClick={() => togglePublish(a._id, a.isPublished)} className={`text-base ${a.isPublished ? 'text-red-500' : 'text-green-600'}`}>{a.isPublished ? 'Unpublish' : 'Publish'}</button>
                </div>
              </td>
            </tr>
          ))}
