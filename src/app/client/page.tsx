  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-3xl font-heading font-bold text-dark">Client Dashboard</h1>
        <Link href="/client/auctions/new" className="btn-primary text-base !py-2.5 gap-2"><Plus className="h-4 w-4" /> New Auction</Link>
      </div>
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
        {[
          ...
          { label: 'Total Revenue', value: formatCurrency(stats.totalRevenue || 0), icon: DollarSign, color: 'text-burgundy bg-burgundy/5' },
          { label: 'Pending Orders', value: stats.pendingOrders || 0, icon: Package, color: 'text-purple-600 bg-purple-100' },
        ].map((s, i) => (
          <div key={i} className="card p-5"><div className="flex items-center justify-between"><div><p className="text-base text-gray-500">{s.label}</p><p className="text-3xl font-bold text-dark mt-1">{s.value}</p></div><div className={`w-12 h-12 rounded-xl flex items-center justify-center ${s.color}`}><s.icon className="h-6 w-6" /></div></div></div>
        ))}
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="card p-6">
          ...
          )) : <p className="text-base text-gray-400">No auctions yet</p>}
        </div>
        <div className="card p-6">
          ...
          )) : <p className="text-base text-gray-400">No orders yet</p>}
        </div>
      </div>
    </div>
  );
