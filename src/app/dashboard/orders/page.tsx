  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
        <div>
          <div className="flex items-center gap-2 mb-3">
             ...
             <span className="text-sm font-black text-burgundy uppercase tracking-[0.4em]">Logistics & Provenance</span>
          </div>
        ...
        <div className="flex items-center gap-4 bg-white border border-gray-100 rounded-2xl px-6 py-3 shadow-xl shadow-black/[0.02]">
           ...
           <div className="text-right">
              ...
              <p className="text-xl font-black text-burgundy">{formatCurrency(orders.reduce((acc, curr) => acc + (curr.totalAmount || 0), 0))}</p>
           </div>
        </div>
      </div>
      ...
      <div className="space-y-6">
        {orders.length === 0 ? (
          <div className="bg-white rounded-[3rem] border border-dashed border-gray-200 py-32 text-center">
            ...
            <p className="text-sm font-bold text-gray-400 uppercase tracking-[0.2em]">Initiate your first acquisition to begin your collection</p>
          ...
          <div className="grid grid-cols-1 gap-6">
            {orders.map((order: any) => (
              <motion.div 
                ...
                className="group bg-white rounded-[2.5rem] p-8 shadow-2xl shadow-black/[0.02] border border-gray-50 hover:border-gold transition-all duration-500 overflow-hidden relative"
              >
                {/* Background Accent */}
                ...
                <div className="relative z-10 grid grid-cols-1 lg:grid-cols-12 gap-10 items-center">
                   ...
                   <div className="lg:col-span-4 flex items-center gap-6">
                      <div className="h-20 w-20 rounded-2xl bg-gray-100 overflow-hidden shrink-0 border border-gray-50 shadow-inner">
                         {order.lot?.images?.[0]?.url ? (
                            <img 
                              src={`${process.env.NEXT_PUBLIC_BACKEND_URL}${order.lot.images[0].url}`} 
                      ...
                      <div>
                         ...
                         <div className="flex items-center gap-2 mt-2">
                            ...
                            <span className="text-base font-black text-burgundy">{formatCurrency(order.totalAmount)}</span>
                         </div>
                      </div>
                   </div>
                   ...
                   <div className="lg:col-span-3 space-y-3">
                      ...
                      <div className={`flex items-center justify-between p-3 rounded-2xl border ${order.shippingStatus === 'delivered' ? 'bg-navy/5 border-navy/10 text-navy' : 'bg-gray-50 border-gray-100 text-gray-400'}`}>
                         <div className="flex items-center gap-2">
      ...
      <div className="bg-navy rounded-[3rem] p-12 text-center relative overflow-hidden">
         ...
         <p className="text-sm font-bold text-white/40 uppercase tracking-[0.4em] max-w-xl mx-auto leading-relaxed">Every acquisition within the Augeo Vault is protected by our strategic audit protocol and verified by the world's leading valuation maisons.</p>
      </div>
    </motion.div>
  );
}