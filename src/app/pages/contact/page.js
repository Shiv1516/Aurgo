    ...
    return (
      <div className="bg-white min-h-screen flex items-center justify-center p-4">
        <div className="max-w-md w-full text-center space-y-8 animate-in fade-in zoom-in duration-500">
           <div className="h-24 w-24 bg-gold/10 rounded-full flex items-center justify-center mx-auto mb-8">
              <CheckCircle2 className="h-12 w-12 text-gold" />
           </div>
           <h2 className="text-4xl font-black text-dark tracking-tighter uppercase">Transmission <span className="text-gold">Received</span></h2>
           ...
           <button 
             ...
             className="btn-primary !px-12 uppercase tracking-widest text-xs font-black"
           >
             Return to Terminal
           </button>
        </div>
    ...
    <div className="bg-white min-h-screen">
      ...
      <section className="py-24 bg-gray-50 border-b border-gray-100 relative overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10 text-center md:text-left">
          <div className="inline-flex items-center gap-2 text-gold text-xs font-black uppercase tracking-[0.4em] mb-4">
          ...
          <h1 className="text-4xl md:text-6xl font-black text-dark tracking-tighter uppercase leading-none mb-6">
            Contact <span className="text-gold italic">Concierge</span>
          </h1>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid lg:grid-cols-2 gap-20">
            ...
            <div className="space-y-12">
               <div>
                  <h2 className="text-3xl font-black text-dark tracking-tighter uppercase mb-8">Guild <span className="text-gold italic">Terminals</span></h2>
                  <div className="grid sm:grid-cols-2 gap-8">
                    {[
                      { icon: Mail, label: "Acquisitions", value: "vault@augeo.global", color: "bg-gold/10" },
                      { icon: Phone, label: "Executive Line", value: "+1 (888) AUGEO-HQ", color: "bg-dark text-gold" },
                      { icon: MapPin, label: "Vault Nexus", value: "New York • London • Dubai", color: "bg-gray-100" },
                      { icon: Clock, label: "Operational Hours", value: "24/7 Global Vigilance", color: "bg-gold/5" }
                    ].map((item, i) => (
                      <div key={i} className="p-6 rounded-3xl border border-gray-50 bg-white shadow-lg shadow-black/5 hover:border-gold/30 transition-all group">
               ...
               <div className="p-10 bg-dark rounded-[3rem] text-white relative overflow-hidden">
                  <div className="absolute top-0 right-0 p-8 opacity-20">
                     <Globe className="h-40 w-40 text-gold" />
                  </div>
                  <div className="relative z-10">
                     ...
                     <div className="flex flex-wrap gap-4">
                        {["New York", "London", "Tokyo", "Dubai", "Singapore"].map((city, i) => (
                          <span key={i} className="px-4 py-2 bg-white/5 border border-white/10 rounded-full text-[10px] font-black uppercase tracking-[0.2em]">{city}</span>
                        ))}
                     </div>
                  </div>
               </div>
            ...
            <div className="relative">
               <div className="bg-white p-10 lg:p-12 rounded-[3.5rem] border border-gray-100 shadow-2xl shadow-black/5 relative z-10">
                  <h3 className="text-2xl font-black text-dark uppercase tracking-tight mb-8">Dispatch a <span className="text-gold italic">Message</span></h3>
                  <form onSubmit={handleSubmit} className="space-y-6">
                    <div className="grid sm:grid-cols-2 gap-6">
                       <div className="space-y-2">
                          ...
                          <input 
                            ...
                            className="w-full px-6 py-4 bg-gray-50 border border-gray-100 rounded-2xl focus:border-gold focus:outline-none transition-all font-bold text-dark placeholder:text-gray-300" 
                          />
                       </div>
                       <div className="space-y-2">
                    ...
                    <div className="space-y-2">
                       <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest ml-1">Inquiry Vector</label>
                       <select 
                         value={form.inquiry} 
                         onChange={e => setForm({...form, inquiry: e.target.value})} 
                         className="w-full px-6 py-4 bg-gray-50 border border-gray-100 rounded-2xl focus:border-gold focus:outline-none transition-all font-bold text-dark appearance-none"
                        >
                    ...
                    <div className="space-y-2">
                       ...
                       <textarea 
                         ...
                         className="w-full px-6 py-4 bg-gray-50 border border-gray-100 rounded-2xl focus:border-gold focus:outline-none transition-all font-bold text-dark placeholder:text-gray-300 resize-none"
                        ></textarea>
                    </div>
                    <button 
                      type="submit" 
                      disabled={isSubmitting} 
                      className="w-full py-5 bg-dark hover:bg-gold text-white hover:text-dark font-black rounded-2xl transition-all shadow-xl shadow-black/10 uppercase tracking-[0.3em] flex items-center justify-center gap-3 group disabled:opacity-50"
                    >
                    ...
                    <p className="text-center text-[10px] text-gray-400 font-bold italic">Average dispatch response time: 57 minutes</p>
                  </form>
               </div>
               
               {/* Background Accent */}
               <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[120%] h-[110%] bg-gold/5 blur-[120px] rounded-full -z-10" />
            </div>
